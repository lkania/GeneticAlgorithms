...
public class ArmorGenFactory implements GenFactory {
	...
	public Gen getRandomGen(int locus) {
		...
		if (locus == 0) {
			int index = (int) (RandomNumbers.getInstance().getRandomNumber() * (GenHeight.MAX_HEIGHT - GenHeight.MIN_HEIGHT) / deltaHeight);
			return new GenHeight(index, deltaHeight);
		} else {
			int maxValue = items.get(locus-1).size();
			int index = (int) (RandomNumbers.getInstance().getRandomNumber() * maxValue);
